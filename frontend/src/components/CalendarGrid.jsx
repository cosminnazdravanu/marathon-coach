...
export default function CalendarGrid({ weeksBack = 5, weeksForward = 5 }) {
  ...
  // Fetch workouts (authenticated GET with cookies)
  const loadTrainingPlan = useCallback(() => {
    fetch(`${API}/plans`, { credentials: "include" })
      ...
      .then((data) => {
        // data is an array of plan objects
        const grouped = data.reduce((acc, e) => {
          const date = e.date;
          const t = e.type || "planned";
    ...
    scrollHandlerRef.current = throttle(() => {
      ...
      if (scrollTop < threshold) {
        ...
        requestAnimationFrame(() => {
          c.scrollTop = scrollTop + (c.scrollHeight - oldH);
        });
  ...
  useEffect(
    () => () =>
      scrollHandlerRef.current.cancel && scrollHandlerRef.current.cancel(),
    []
  );

  // Move workout → PATCH with CSRF + cookies → reload
  const moveWorkout = useCallback(
    async (id, _fromDate, toDate) => {
      try {
  ...
  return (
    <div className="w-full mx-auto py-4">
            ...
            <>
              ...
              >
                ...
                <span className="font-medium text-gray-700">
                  {format(new Date(pickerValue), "MMMM yyyy")}
                </span>
                <ChevronDownIcon
              ...
              >
                <PopoverPanel className="absolute z-10 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg p-4 w-[300px]">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <button
                      onClick={() =>
                        setPickerValue(
                          format(
                            subMonths(new Date(pickerValue), 1),
                            "yyyy-MM-dd"
                          )
                        )
                      }
                    ...
                    <div className="flex space-x-2">
                      ...
                      >
                        {Array.from({ length: 12 }, (_, i) => {
                          const m = new Date(0, i).toLocaleString("default", {
                            month: "short",
                          });
                          return (
                      ...
                      >
                        {(() => {
                          ...
                          return Array.from({ length: 21 }, (_, i) => year - 10 + i).map(
                            (y) => (
                    ...
                    <button
                      onClick={() =>
                        setPickerValue(
                          format(
                            addMonths(new Date(pickerValue), 1),
                            "yyyy-MM-dd"
                          )
                        )
                      }
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {eachDayOfInterval({
                      start: startOfWeek(startOfMonth(new Date(pickerValue)), {
                        weekStartsOn: 1,
                      }),
      ...
      <div
        ...
        className="h-[calc(100vh-200px)] overflow-auto px-4 bg-white"
      >
            ...
            return (
              <div
                ...
                onDrop={(e) => {
                  ...
                  try {
                    const { id, date: from } = JSON.parse(
                      e.dataTransfer.getData("application/json")
                    );
                    if (from !== iso) moveWorkout(id, from, iso);
                  } catch {}
                }}
              >
                ...
                <div className="flex-1 flex flex-col space-y-2 overflow-y-auto">
                  {day.planned.map((w) => (
                    <WorkoutCard key={w.id} workout={w} onEdit={handleEdit} />
                  ))}
                  {day.strava && (
                    <WorkoutCard
                      workout={{ ...day.strava, date: iso, type: "strava" }}
                      onEdit={null}
                    />
                  )}
                  <div
                    onClick={() => handleAdd(iso)}
                    className="hidden group-hover:flex items-center justify-center p-2 mb-2 bg-white shadow rounded-lg text-gray-400 hover:text-blue-500 cursor-pointer transition"
                  >
                    <FiPlus size={20} />
                  </div>
                </div>
